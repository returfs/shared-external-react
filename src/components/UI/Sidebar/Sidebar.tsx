...
import { twoHundredEightHundredBorderColors } from '../../../styles/colors/Border/Border';
import { ColorKey } from 'src/styles';
import { useSidebarResize } from 'src/hooks/use-sidebar-resize';
import { mergeButtonRefs } from 'src/lib/merge-button-refs';
      ...
      return (
        <div
          className={cn(
            ...
            twoHundredEightHundredBorderColors[colorKey],
            className,
          )}
          ref={ref}
          {...props}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isDragging.current ? '[[data-side=left]_&]:cursor-ew-resize' : '',
        className,
      )}
      {...props}
    />
  ...
  cva(
    'peer/menu-button ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground flex w-full items-center gap-2 overflow-hidden rounded-lg p-2 text-left text-sm outline-none transition-[width,height,padding] focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium group-data-[collapsible=icon]:!size-9 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-5 [&>svg]:shrink-0',
    {
      variants: {
        ...
        size: {
          default: 'h-[33px] text-sm',
          child: 'h-[30px] text-sm',
          sm: 'h-7 text-xs',
...
const SidebarMenuButton = React.forwardRef<
  ...
  } & VariantProps<ReturnType<typeof sidebarMenuButtonVariants>>
>(
  (
    {
      asChild = false,
    ...
    const { colorKey } = useTheme();
    const sbv = sidebarMenuButtonVariants(colorKey);
